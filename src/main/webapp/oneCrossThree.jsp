                            ...
                            myChart.setOption(
                                (option = {
                                    ...
                                    legend: {
                                        bottom: 10,
                                        left: 'center',
                                        data: ['Dow-Jones index', 'MA10', 'MA20', 'MA30']
                                    },
                                    tooltip: {
                                        trigger: 'axis',
                                        axisPointer: {
...
//
        ...
        function oneCrossThree(data) {
            //open ,close ,l,h
            let day1 = calculateMA(10, data);
            let day2 = calculateMA(20, data);
            let day3 = calculateMA(30, data);
            let buy = [];
            let list = data.values;
            let isUP = false;
            for (let i = 0; i < data.values.length; i++) {
                if (!isUP) {
                    if (list[i][3] > day1[i] && list[i][3] > day2[i] && list[i][3] > day3[i]) {
                        if (list[i][2] < day1[i] && list[i][2] < day2[i] && list[i][2] < day3[i]) {
                            isUP = true;
                            buy.push(list[i][3] + 50);
                            continue;
                        }
                    }
                    buy.push(list[i][2] - 50);
                }
                if (isUP) {
                    if (list[i][2] < day2[i]) {
                        isUP = false
